...
@Component
@RabbitListener(queues = { ServiceMQConstant.NOTICE_TEST_GROUP })
public class CryptoCoinNoticeTestGroupMsgReceiver extends CommonMessageQueueReceiverService {
	...
	public void process(String messageStr, Channel channel, Message message) throws IOException {
		try {
			...
			telegramService.sendMessageByChatRecordId(TelegramBotType.CRYPTO_COIN_LOW_PRICE_NOTICE_BOT, dto.getMsg(),
					TelegramStaticChatID.NOTICE_TEST_GROUP_ID);
		} catch (Exception e) {
			log.error("mq error, " + ServiceMQConstant.NOTICE_TEST_GROUP + ", e:" + e.getLocalizedMessage());
