...
public class AccountInfoServiceImpl extends CommonService implements AccountInfoService {
	...
	public AccountRegistResult accountRegistration(AccountInfoRegistDTO dto) {
		...
		/*
		 * TODO FIXME if (holder == null) { return holderRegist?? }
		 */
	...
	@Override
	public GetAccountNumberAndAliasListResult findCurrentAccountNumberListByCondition(
			FindAccountInfoByConditionDTO dto) {
	...
	@Override
	public List<AccountInfoWithBankInfo> getAccountInfoWithBankInfoByCondition(Long holderId,
			GetAccountListByConditionParam cp) {
	...
	public List<AccountInfoWithBankInfo> accountUsedQuotaStatistics(List<AccountInfoWithBankInfo> accountInfoList) {
		...
		List<AccountStatisticsByBankIdBO> statisticsList = accountInfoStatisticsService
				.accountStatisticsByBankId(accountInfoList);
	...
	private void setAvaliableQuotaByStatisticResult(List<AccountStatisticsByBankIdBO> statisticsList,
			List<AccountInfoWithBankInfo> accountInfoList) {
	...
	@Override
	@Transactional(value = "cxTransactionManager", rollbackFor = Exception.class)
	public InsertTransationResult insertTradingRecorderSelective(InsertNewTransationDTO insertNewTransationDTO)
			throws Exception {
			...
			try {
				if (AccountType.creditAccount.getCode().equals(targetAccount.getAccountType())) {
					...
					if (insertNewTransationDTO.getFixCreditQuota() != null) {
						...
						transationAmount = targetQuota.subtract(targetAccount.getAccountBalance())
								.subtract(targetAccount.getTotalCreditQuota());
					}
		...
		InsertTradingRecorderResult InsertTradingRecordResult = tradingController
				.insertTradingRecorderSelective(insertNewTransationDTO, targetAccount.getAccountId());

		TradingRecorder tradingRecord = tradingController
				.getTradingRecorderById(InsertTradingRecordResult.getNewTradingId());
	...
	public boolean checkAccountBelongUserByTailNumber(String accountTailNumber) {
		List<AccountHolder> holderList = accountHolderController.getCurrentHolders();
		if (null == holderList || holderList.size() == 0 || null == accountTailNumber
				|| accountTailNumber.length() == 0) {
			return false;
		}
...
//		HashMap<Integer, > resultMap = new HashMap<Integer, SummaryAccountsByBankId>();
	...
	@Override
	@Transactional(value = "cxTransactionManager", rollbackFor = Exception.class)
	public int modifyAccountInfoTemproraryCreditsVaildDate(String temproraryCreditsVaildDate, String accountNumber,
			boolean isAdmin) {
	...
	@Override
	@Transactional(value = "cxTransactionManager", rollbackFor = Exception.class)
	public CommonResult modifyCreditsQuota(ModifyCreditsQuotaDTO dto) {
		CommonResult r = new CommonResult();
		if (dto.getAccountNumber() == null || dto.getNewCreditsQuota() == null
				|| !checkAccountNumberBelongUser(dto.getAccountNumber())) {
			return r;
		}
	...
	public GetAccountListResult accountInfoWithBankInfoList(GetAccountListByConditionParam param) {
		...
		List<AccountInfoWithBankInfo> accountList = getAccountInfoWithBankInfoByCondition(
				0L + holderList.get(0).getId(), param);
		result.setIsSuccess();
	...
	private String getAccountInfoInfos(AccountInfo po) {
		return "" + po.getAccountId() + po.getAccountHolderId() + po.getAccountNumber() + po.getAccountAlias()
				+ po.getAccountType() + po.getAccountAffiliation() + po.getVaildDate() + po.getBankId()
				+ po.getBankUnionId() + po.getAccountBalance() + po.getCreditsQuota() + po.getTemproraryCreditsQuota()
				+ po.getTemproraryCreditsVaildDate() + po.getRemark() + po.getIsDelete() + po.getCreateTime();
	}
