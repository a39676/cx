...
public class CryptoCompareWSClient extends CommonService {
	...
	public static void main(String[] args) throws IOException {
		String uriStr = "wss://streamer.cryptocompare.com/v2?api_key=420c0cfe267a0df93417e95bc9f3446bddfb73bde91ac227fdcaaf982be910dc";

		JSONObject j = new JSONObject();
		j.put("action", "SubAdd");
		JSONArray subs = new JSONArray();
		subs.add("5~CCCAGG~BTC~USD");
		subs.add("5~CCCAGG~ETH~USD");
		subs.add("5~CCCAGG~FIL~USD");
		j.put("subs", subs);

		String addSub = j.toString();
...
//			wss.sendClose();
	...
	private WebSocket createWebSocket(CryptoCompareSocketConfigBO configBO) {
		String uriStr = configBO.getUri() + "?apk_key=" + configBO.getApiKey();
		try {
...
//					TODO
	...
	private void addSubscription(WebSocket ws, CryptoCompareSocketConfigBO bo) {
		...
		for(String cryptoCoinCode : bo.getTragetCoins()) {
	 ...
	 * 1 = active, 0 = inactive
	 */
	private void setSocketLiveFlag(String intFlag) {
	...
	public boolean getSocketLiveFlag() {
		...
		String lastActiveTimeStr = constantService.getSysValByName(CryptoCompareConstant.SOCKET_LAST_ACTIVE_TIME_REDIS_KEY);
		LocalDateTime lastActiveTime = localDateTimeHandler.stringToLocalDateTimeUnkonwFormat(lastActiveTimeStr);
		return lastActiveTime.plusSeconds(CryptoCompareConstant.SOCKET_INACTIVE_JUDGMENT_SECOND).isAfter(LocalDateTime.now());
	}
	
	private void refreshLastActiveTime() {
		constantService.setValByName(CryptoCompareConstant.SOCKET_LAST_ACTIVE_TIME_REDIS_KEY, localDateTimeHandler.dateToStr(LocalDateTime.now()));
	}
	...
	private CryptoCompareSocketConfigBO getConfig() {
		...
		String cryptoCompareParameterSavingFolderPath = systemParameterSavingFolderPath + File.separator + CryptoCompareConstant.PARAM_STORE_PATH;
	...
	private CryptoCoinPriceCommonDataBO buildCommonDataFromMsg(String msg) {
		...
		try {
			...
			try {
				Date tradDate = new Date(j.getLong("LASTUPDATE"));
