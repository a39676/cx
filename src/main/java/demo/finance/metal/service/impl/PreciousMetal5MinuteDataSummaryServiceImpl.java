...
@Service
public class PreciousMetal5MinuteDataSummaryServiceImpl extends PreciousMetalCommonService
		implements PreciousMetal5MinuteDataSummaryService {
	...
	public void cacheDataTo5Minute() {
		int minuteStep = 5;
		LocalDateTime endTime = nextStepStartTimeByMinute(LocalDateTime.now(), minuteStep);
	...
	@Override
	public CommonResult cacheDataTo5Minute(MetalType metalType, LocalDateTime startTime, LocalDateTime endTime) {
...
//		当前时间区间无汇总数据, 汇总后更新, 否则直接更新
	...
	private MetalPrice5minute findSummaryPO(MetalType metalType, LocalDateTime startTime, LocalDateTime endTime) {
		...
		example.createCriteria().andIsDeleteEqualTo(false).andMetalTypeEqualTo(metalType.getCode())
