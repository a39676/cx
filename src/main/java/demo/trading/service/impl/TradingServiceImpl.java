...
public class TradingServiceImpl extends CommonService implements TradingService {
	...
	public List<CommonTransationParties> getCurrentCommonTransation(Long holderId, Integer limit) {
		...
		List<CommonTransationParties> commonTransationList = holderCommonTransationCustomMapper.getCurrentCommonTransation(holderId, limit);
...
//		List<File> fileList = new ArrayList<File>();
		for(File tmpFile : files) {
			if(tmpFile.isFile() && tmpFile.getName().endsWith(".csv") && tmpFile.getName().substring(0, tmpFile.getName().lastIndexOf(".")).matches("\\d{16,19}")) {
...
//		Date now = new Date();
//			
//		Date transationDate = DateUtilCustom.stringToDateUnkonwFormat(String.valueOf(jsonInput.get("transationDate")));
//		String transationParties = String.valueOf(jsonInput.get("transationParties"));
//		BigDecimal transationAmount = null;
//		String accountNumber = String.valueOf(jsonInput.get("accountNumber"));
//		if(!NumericUtilCustom.matchPositiveInteger(accountNumber)) {
//			return null;
//		}
//		
//		try {
//			if(jsonInput.get("transationType").equals(TransationType.transationTypePay.getCode())) {
//				transationAmount = BigDecimal.ZERO.subtract(new BigDecimal(String.valueOf(jsonInput.get("transationAmount"))));
//				
//			} else if (jsonInput.get("transationType").equals(TransationType.transationTypeIncome.getCode())) {
//				transationAmount = new BigDecimal(String.valueOf(jsonInput.get("transationAmount")));
//				
//			} else {
//				return null;
//			}
...
//		tradingRecorder.setAccountId(Long.parseLong(String.valueOf(jsonInput.get("accountId"))));
...
//	}
	...
	private TradingRecorder buildTradingRecorderFrom(Long newTradingRecordId, InsertNewTransationParam p, Long accountId) {
